...
check_root() {
    if [[ $EUID != 0 ]]; then
        echo -e "${Error} You currently do not have ROOT privileges. Please use ${Green_background_prefix}sudo${Font_color_suffix} to run this script."
        exit 1
    fi
}
...
install_env_and_full_node() {
    ...
    brew install curl tar wget clang pkg-config openssl jq git docker
